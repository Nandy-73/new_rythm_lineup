...
// ─── Draw: Checkpoints ────────────────────────────────────────────────────────
function drawCheckpoints(ctx, path, checkpoints, W, H, ts) {
  checkpoints.forEach(cp => {
    ...
    if (cp.active) {
      ...
      aura.addColorStop(0, `rgba(255,224,102,${0.12 * pulse})`);
      ...
      [-4, 4].forEach(dx => {
        ctx.beginPath();
        ctx.moveTo(sx + dx, py - halfH * 0.7);
        ctx.lineTo(sx + dx, py + halfH * 0.7);
        ctx.stroke();
      });
      ctx.restore();
  ...
  useEffect(() => {
    ...
    function frame(ts) {
      ...
      s.checkpoints.forEach(cp => {
        ...
        if (!cp.active && dist2 >= 0 && dist2 < W * 0.06) {
          cp.active      = true;
      ...
      drawCheckpoints(ctx, currentPath, s.checkpoints, W, H, ts);
